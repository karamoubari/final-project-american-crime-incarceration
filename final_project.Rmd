...
>>>>>>> 6ff7ef80bcd4546f5363b4a0f5939e29eec382ef
...
```
<<<<<<< HEAD
In 2012 the FBI changed their definition of rape from "The carnal knowledge of
a female forcibly and against her will,” to, “'Penetration, no matter how 
...
We wanted to see how the change in definition affected the number of prisoners 
convicted for rape. Based on the graph, it seems like after the new definition 
was implemented, the number of prisoners who are incarcerated for rape 
increased. We will perform tests to find the p-value and try to determine if 
this is correlation or a causal relationship.
...
The creation and enforcement of policy has a greater impact on who is 
incarcerated and how many people are incarcerated for each crime than the 
actual rate of crime. Despite public perception assuming the opposite 
relationship. Many times the policies can be used to adjust a narrative to 
support a political movement or idea at the time. Crime rates can increase when 
policy is adjusted to include more crime as seen with the change in defintion 
for rape. This is not to say that this change contributed significantly to mass
incarceration or that the change in definition was unjust. Rather, this example
in our data exemplifies the impact of policies on incarceration rather than 
crime rate.
...
>>>>>>> ac84c99db0d2df0cf1263633692b4d8c66110e9d
crime_props <- crime %>%
  filter(jurisdiction != "FEDERAL") %>%
...
```{r proportion-of-crimes-including-larcency-2, echo = FALSE, warning=FALSE, message=FALSE, eval = FALSE}
ggplot(data = crime_props, mapping = aes(x = year)) +
...
```{r plot-violent-nonviolent-crimes, echo = FALSE, eval = FALSE}
ggplot(data = crime_props, mapping = aes(x = year)) +
...
```{r new-dataset, echo = FALSE, warning=FALSE, message=FALSE, eval = FALSE}
...
=======
```{r line-graph, eval = FALSE, echo = FALSE}
>>>>>>> 6ff7ef80bcd4546f5363b4a0f5939e29eec382ef
...
```{r reg-analysis-graphs, eval=FALSE, echo=FALSE, message=FALSE, warning=FALSE}
